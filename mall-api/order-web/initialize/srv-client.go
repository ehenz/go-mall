...
func InitSrvClient() {
	...
	orderConn, err := grpc.Dial(
		fmt.Sprintf("consul://%s:%d/%s?wait=14s", c.ConsulConfig.Host, c.ConsulConfig.Port, c.OrderSrvConfig.Name),
		grpc.WithInsecure(),
		grpc.WithDefaultServiceConfig(`{"loadBalancingPolicy": "round_robin"}`), // 轮询
	)
	if err != nil {
		zap.S().Errorw("订单服务初始化失败", "msg", err.Error())
	}
	orderSrvClient := pb.NewOrderClient(orderConn)
	...
	if err != nil {
		zap.S().Errorw("库存服务初始化失败", "msg", err.Error())
	}
